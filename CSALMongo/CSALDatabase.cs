...
namespace CSALMongo {
    ...
    public class CSALDatabase {
        ...
        protected object RawContents(BsonDocument doc) {
            ...
            string lessonid = doc.GetValue("LessonID", "").AsString.Trim().ToLower();

            // need to add more: based on rules, change it with every specific lesson

            // modified by Qiong
            if (lessonid.Contains("lesson8"))
            {
                ret.CorrectAnswers = 1;
                //Check transitions for data
                var transitionsscore = Util.ExtractArray(doc, "Transitions");
                foreach (var trans in transitionsscore)
                {
                    ...
                    foreach (var action in actions)
                    {
                        ...
                        string data = actionDoc.GetValue("Data").AsString.Trim().ToLower();

                        if (act == "adduserscore" && data == "1")
                        {
                            ret.CorrectAnswers = 1;
                        }
                        else if (act == "getmediafeedback" && data == "sagoodanswer")
                        {
